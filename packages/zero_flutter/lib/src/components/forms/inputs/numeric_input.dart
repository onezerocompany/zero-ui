...
class NumericInput<NumberType extends num> extends InputField<NumberType> {
  const NumericInput(
    super.state, {
    super.key,
  ...
  Widget fieldBuilder(
    ...
    NumberType numberFromString(String value) {
      try {
        final parsed = numberFormatter.parse(value);
        // case double to int if needed
        if (NumberType == int) {
                    ...
                    child: ConstrainedBox(
                      ...
                      child: Focus(
                        ...
                        child: TextField(
                          ...
                          onSubmitted: (value) {
                            final number =
                                numberFromString(value) * (currency ? 100 : 1);
                            if (NumberType == int) {
                              onSubmittedField(number.toInt() as NumberType);
                            } else if (NumberType == double) {
                              onSubmittedField(number.toDouble() as NumberType);
                            } else {
                          ...
                          onChanged: (value) {
                            ...
                            if (NumberType is int) {
                              ...
                              state.silentValueUpdate(number as NumberType);
                            }
                          },
                        ),
                      ),
